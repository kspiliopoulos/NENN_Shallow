...
def load_cifar_data(batch_size):
    """Return train and test data loaders for CIFAR10 dataset
...
def generate_file_name(dataset_name, model_name, criterion_name, gamma,
                       hidden_units, epochs, batch_size):
    """Returns a file name specifying parameters, e.g.,
        'mnist_mlp_ce_g06_h1000_e500_b20'
    ...
    if gamma is not None:
        g = 'g{0}'.format(str(gamma).replace('.',''))
        parts += [g]
    h = 'h{0}'.format(hidden_units)
    e = 'e{0}'.format(epochs)
